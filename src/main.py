...
# Data Awal (Raw Data)
raw_data = {
    "Jarak Kedatangan (menit)": [0, 1, 2, 3, 4],
    "Frekuensi Kedatangan Konsumen": [0.20] * 5,  
    "Lama Pelayanan (menit)": list(range(5, 25)),
    "Frekuensi Pelayanan Konsumen": [0.50] * 20,  
}
...
with tabs[0]:
    ...
    random_method = st.selectbox("Metode Bilangan Acak", ["Random Default", "LCG"])
    
    # LCM/LCG Parameters jika dipilih
    if random_method in ["LCG"]:
        if random_method == "LCG":
    ...
    with main_tabs[0]:  # "📈 Dashboard" tab
        ...
        with col1:
            ...
            with col4:
                ...
                st.session_state.rata_rata_tis = rata_rata_tis
            with col5:
                rata_rata_pelayanan = simulation_df["Lama Pelayanan (menit)"].mean()
                st.metric("Rata-rata pelayanan", f"{rata_rata_pelayanan:.1f} menit")
                st.session_state.rata_rata_pelayanan = rata_rata_pelayanan
